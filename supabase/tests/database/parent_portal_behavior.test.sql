...
select extensions.results_eq(
  ...
  $$values (false, null::text, 0)$$,
  'B02. repeated runtime login remains generic and sessionless'
);

select extensions.is(
  (select count(*) from public.validate_parent_access_session(repeat('c', 64))),
  0::bigint,
  'B03. unknown runtime session validates to no rows'
);
select extensions.is(
  (select count(*) from public.get_parent_published_reports(repeat('c', 64))),
  0::bigint,
  'B04. unknown runtime session lists no reports'
);
select extensions.is(
  ...
  0::bigint,
  'B05. unknown runtime session reads no report detail'
);
select extensions.is(
  ...
  0::bigint,
  'B06. unknown runtime session reads no artifact descriptor'
);
...
select extensions.throws_ok(
  ...
  '42501', null,
  'B07. anonymous runtime login remains denied'
);
select extensions.throws_ok(
  $$select * from public.get_parent_published_reports(repeat('c',64))$$,
  '42501', null,
  'B08. anonymous runtime report listing remains denied'
);
select extensions.throws_ok(
  $$select * from public.parent_access_rate_limits$$,
  '42501', null,
  'B09. anonymous runtime throttle table access remains denied'
);
