...
select extensions.is((select snapshot_checksum from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),(select snapshot_checksum from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),'B10. reader returns persisted checksum');
...
select extensions.throws_ok($$select * from public.confirm_promotion_decision((select decision_id from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),99,'PROMOTED')$$,'PT409','PROMOTION_DECISION_VERSION_CONFLICT','B14. confirmation version is checked');
select extensions.lives_ok($$select * from public.confirm_promotion_decision((select decision_id from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),1,'PROMOTED')$$,'B15. confirmation executes');
select extensions.is((select final_decision::text from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),'PROMOTED','B16. final decision is stored');
select extensions.is((select was_overridden from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),false,'B17. matching decision is not an override');
select extensions.throws_ok($$select * from public.confirm_promotion_decision((select decision_id from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),1,'REPEAT_CONFIRMED')$$,'55000','PROMOTION_DECISION_CONFIRMED_IMMUTABLE','B18. confirmation is immutable');
select extensions.lives_ok($$select * from public.reopen_promotion_decision((select decision_id from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),1,'Review evidence')$$,'B19. reopen executes');
select extensions.is((select max(version) from public.list_promotion_decision_history('e1d00000-0000-4000-8000-000000000001')),2,'B20. reopen increments version');
select extensions.is((select count(*) from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),1::bigint,'B21. one current decision remains');
select extensions.is((select count(*) from public.list_promotion_decision_history('e1d00000-0000-4000-8000-000000000001')),2::bigint,'B22. prior decision is retained');
select extensions.throws_ok($$select * from public.reopen_promotion_decision((select decision_id from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),1,'Review evidence')$$,'PT409','PROMOTION_DECISION_VERSION_CONFLICT','B23. stale reopen is rejected');
select extensions.lives_ok($$select * from public.confirm_promotion_decision((select decision_id from public.list_promotion_recommendations('e1500000-0000-4000-8000-000000000001','e1600000-0000-4000-8000-000000000001')),2,'REPEAT_CONFIRMED')$$,'B24. repeat confirmation executes');
