-- Stage 11 additive acceptance corrections. Migrations 23-26 remain unchanged.
...
begin
  ...
  select grade.* into selected_grade
  from public.grade_levels grade
  where grade.id = target_grade_level_id
  ...
  where section.academic_year_id = selected_year.id
    and section.grade_level_id = selected_grade.id
    and section.is_active;
  select count(*) into student_population
  from public.enrollments enrollment
  join public.class_sections section on section.id = enrollment.class_section_id
  where enrollment.academic_year_id = selected_year.id
    and enrollment.status in ('ACTIVE', 'REPEATING')
    and section.academic_year_id = selected_year.id
    and section.grade_level_id = selected_grade.id
    and section.is_active;
  ...
  with expected_scopes as (
    ...
    join public.grade_level_subjects mapping
      on mapping.grade_level_id = selected_grade.id
    where section.academic_year_id = selected_year.id
      and section.grade_level_id = selected_grade.id
      and section.is_active
  ), latest_sources as (
    select expected.class_section_id, expected.subject_id,
      sheet.id as mark_sheet_id, sheet.workflow_status,
      row_number() over (
        partition by expected.class_section_id, expected.subject_id
        order by sheet.version desc nulls last, sheet.id desc nulls last
      ) as source_rank
    from expected_scopes expected
    left join public.mark_sheets sheet
      on sheet.term_id = target_term_id
     and sheet.class_section_id = expected.class_section_id
     and sheet.subject_id = expected.subject_id
  )
  select count(*)::bigint,
    count(*) filter (where mark_sheet_id is not null and source_rank = 1)::bigint,
    count(*) filter (where mark_sheet_id is null and source_rank = 1)::bigint,
    count(*) filter (
      where mark_sheet_id is not null
        and source_rank = 1
        and workflow_status <> 'LOCKED'
    )::bigint
  into expected_scopes, sources, missing, unlocked
  from latest_sources
  where source_rank = 1;

  select count(*) into scale_count
  from public.grading_scales scale
  where scale.school_id = actor.school_id
    and (scale.academic_year_id is null or scale.academic_year_id = selected_year.id)
    and (scale.grade_level_id is null or scale.grade_level_id = selected_grade.id)
    and scale.is_active and scale.retired_at is null;
  select count(*) into ranking_count
  from public.ranking_rules rule
  where rule.school_id = actor.school_id
    and (rule.academic_year_id is null or rule.academic_year_id = selected_year.id)
    and (rule.grade_level_id is null or rule.grade_level_id = selected_grade.id)
    and rule.is_active and rule.retired_at is null;
  select count(*) into classification_count
  ...
  where run.term_id = target_term_id
    and run.grade_level_id = target_grade_level_id
  ...
  if latest_run.id is not null then
    current_checksum := internal.results_input_checksum(
      ...
      latest_run.grading_scale_id,
      latest_run.ranking_rule_id,
      latest_run.aggregate_classification_scale_id
    );
  elsif scale_count = 1 and ranking_count = 1 then
    select scale.id into selected_scale_id
    from public.grading_scales scale
    where scale.school_id = actor.school_id
      and (scale.academic_year_id is null or scale.academic_year_id = selected_year.id)
      and (scale.grade_level_id is null or scale.grade_level_id = selected_grade.id)
      and scale.is_active and scale.retired_at is null;
    select rule.id into selected_ranking_id
    from public.ranking_rules rule
    where rule.school_id = actor.school_id
      and (rule.academic_year_id is null or rule.academic_year_id = selected_year.id)
      and (rule.grade_level_id is null or rule.grade_level_id = selected_grade.id)
  ...
  term_status := selected_term.status;
  term_name := selected_term.name;
  academic_year_name := selected_year.name;
  grade_level_id := selected_grade.id;
  grade_name := selected_grade.name;
  expected_class_subject_scopes := expected_scopes;
  source_sheet_count := sources;
  missing_source_scopes := missing;
  non_locked_latest_scopes := unlocked;
  latest_run_id := latest_run.id;
  latest_version := latest_run.version;
  calculation_present := latest_run.id is not null;
  applicable_grading_scale_count := scale_count;
  applicable_ranking_rule_count := ranking_count;
  applicable_classification_scale_count := classification_count;
  current_authoritative_input_checksum := current_checksum;
  latest_run_input_checksum := latest_run.input_checksum;
  up_to_date := latest_run.id is not null
    and current_checksum is not null
    and latest_run.input_checksum = current_checksum
    and missing = 0
    and unlocked = 0;
  return next;
end;
...
grant execute on function public.get_results_calculation_readiness(uuid, uuid) to authenticated;
revoke execute on function public.get_results_calculation_readiness(uuid, uuid) from public, anon;


-- Re-declare the calculation function only to correct deterministic ORDINAL ordering.
...
begin
  ...
  execute format($sql$ update %I set ranking_eligible = ranking_metric is not null and complete_subject_count >= $1 and ($2 or is_complete) where true $sql$, student_table) using minimum_subjects, include_incomplete;
  execute format($sql$
    with ranked as (select student.*, count(*) over(partition by student.class_section_id, student.ranking_metric)::integer class_ties, count(*) over(partition by student.ranking_metric)::integer grade_ties, dense_rank() over(partition by student.class_section_id order by (case when $1::text='DESC' then student.ranking_metric end) desc nulls last, (case when $1::text='ASC' then student.ranking_metric end) asc nulls last) class_dense, rank() over(partition by student.class_section_id order by (case when $1::text='DESC' then student.ranking_metric end) desc nulls last, (case when $1::text='ASC' then student.ranking_metric end) asc nulls last) class_competition, row_number() over(partition by student.class_section_id order by (case when $1::text='DESC' then student.ranking_metric end) desc nulls last, (case when $1::text='ASC' then student.ranking_metric end) asc nulls last, upper(btrim(person.admission_number)), student.enrollment_id) class_ordinal, dense_rank() over(order by (case when $1::text='DESC' then student.ranking_metric end) desc nulls last, (case when $1::text='ASC' then student.ranking_metric end) asc nulls last) grade_dense, rank() over(order by (case when $1::text='DESC' then student.ranking_metric end) desc nulls last, (case when $1::text='ASC' then student.ranking_metric end) asc nulls last) grade_competition, row_number() over(order by (case when $1::text='DESC' then student.ranking_metric end) desc nulls last, (case when $1::text='ASC' then student.ranking_metric end) asc nulls last, upper(btrim(person.admission_number)), student.enrollment_id) grade_ordinal from %I student join public.enrollments enrollment on enrollment.id = student.enrollment_id join public.students person on person.id = enrollment.student_id where student.ranking_eligible)
  ...
  execute format($sql$
    with ranked as (
      select subject.enrollment_id, subject.class_section_id, subject.subject_id,
        ...
        row_number() over(partition by subject.class_section_id, subject.subject_id order by subject.subject_score desc, upper(btrim(person.admission_number)), subject.enrollment_id) ordinal_position
      from %I subject
      join public.enrollments enrollment on enrollment.id = subject.enrollment_id
      join public.students person on person.id = enrollment.student_id
